...
public class FlyBanner extends RelativeLayout {
    ...
    //指示点资源
    private int mPointDrawableResId = R.drawable.selector_bgbanner_point;
    //指示容器背景
    ...
    private int length;
    private int type; // 1 欢迎界面引导页   2  其他
    ...
    private void setLayout(Context context) {
        ...
        //设置内边距
        pointContainerRl.setPadding(0, dp2px(context, 10), 0, dp2px(context, 40));
     ...
     * @param images
     */
    public void setImages(List<Integer> images, int type) {
        //加载本地图片
        mIsImageUrl = false;
        this.mImages = images;
        if (images.size() <= 1)
    ...
    private class FlyPageAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, final int position) {
            ...
            if (mIsImageUrl) {
                String imgUrl = mImageUrls.get(toRealPosition(position));
                Picasso.with(App.mContext)
                        .load(imgUrl).config(Bitmap.Config.RGB_565).into(imageView);
            } else {
    ...
    private void switchToPoint(final int currentPoint) {
        ...
        for (int i = 0; i < 4; i++) {
            if (mPointRealContainerLl != null) {
                View childAt = mPointRealContainerLl.getChildAt(i);
                if (childAt != null) {
